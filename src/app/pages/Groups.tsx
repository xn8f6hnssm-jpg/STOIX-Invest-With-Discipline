...
import { Users, Plus, Search, Lock, Unlock, Copy, Check, DollarSign, TrendingUp, Crown } from 'lucide-react';
...
export function Groups() {
  ...
  const handleJoinByInviteCode = () => {
    ...
    // For paid groups, process payment
    if (group.type === 'paid' && group.price) {
      const confirmed = confirm(`This group costs $${group.price}/month. Join now?`);
      if (!confirmed) return;

      // Process payment: 95% to creator, 5% platform fee
      const creatorEarnings = group.price * 0.95;
      const platformFee = group.price * 0.05;
      ...
      if (!creatorCredits) {
        ...
        storage.addUserCredits(creatorCredits);
      }
      ...
      storage.addCreditTransaction({
        ...
        source: `Group payment from ${currentUser.username} (${group.name})`,
        status: 'completed',
      });

      // Update creator balance
      storage.updateUserCredits(group.creatorId, {
        balance: (creatorCredits.balance || 0) + creatorEarnings,
        totalEarned: (creatorCredits.totalEarned || 0) + creatorEarnings,
      });
    }
  ...
  const handleRequestToJoin = (groupId: string) => {
    ...
    const existingRequest = storage.getJoinRequests().find(
      r => r.groupId === groupId && r.userId === currentUser.id && r.status === 'pending'
    );
  ...
  if (!currentUser) {
    return <div className="p-6">Please log in to view groups</div>;
  }
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Join communities, share strategies, and compete in challenges
          </p>
        </div>
        <div className="flex gap-2">
          <Button onClick={() => navigate('/app/credits')} variant="outline">
            <DollarSign className="w-4 h-4 mr-2" />
            Credits
          </Button>
          <Button onClick={() => setShowJoinByCodeModal(true)} variant="outline">
          ...
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Create your own group or join with an invite code
              </p>
            </CardContent>
          </Card>
        ) : (
      ...
      <Dialog open={showCreateModal} onOpenChange={setShowCreateModal}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Build a community around your trading strategy
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              <Input
                placeholder="e.g., ICT Traders Community"
                value={newGroupName}
                ...
                <SelectContent>
                  ...
                  <SelectItem value="paid">
                    <div className="flex items-center gap-2">
                      ...
                      Paid Group (Members pay to join)
                    </div>
                  </SelectItem>
                </SelectContent>
              </Select>
            ...
            {newGroupType === 'paid' && (
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  You earn 95% (${ (parseFloat(newGroupPrice) * 0.95).toFixed(2) }) • Platform fee 5%
                </p>
              </div>
            )}
            <div className="flex items-center gap-2">
      ...
      <Dialog open={showJoinByCodeModal} onOpenChange={setShowJoinByCodeModal}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Enter the 6-character invite code to join instantly
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
