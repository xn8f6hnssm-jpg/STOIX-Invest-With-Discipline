...
export function Groups() {
  ...
  const loadGroups = async () => {
    try {
      ...
      if (data && !error) {
        ...
        setAllGroups(storage.getGroups());
      }
    } catch {
      setAllGroups(storage.getGroups());
    } finally {
  ...
  const handleCreateGroup = async () => {
    ...
    storage.addGroup(newGroup as any);
  ...
  const handleJoinByInviteCode = async () => {
    ...
    if (group.type === 'paid' && group.price) {
      ...
      let creatorCredits = storage.getUserCredits().find((c: any) => c.userId === group.creatorId);
