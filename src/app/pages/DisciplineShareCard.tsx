...
function getDateBounds(range: Range): { from: Date | null; todayStr: string } {
  ...
  return { from: null, todayStr }; // overall — no filter
}

function inRange(dateStr: string, from: Date | null): boolean {
  if (!from) return true; // overall
  const d = new Date(dateStr + 'T00:00:00');
  return d >= from;
}

export function DisciplineShareCard({ range = 'overall' }: { range?: Range }) {
  ...
  if (range === 'overall') {
    // Use user object totals — most accurate (includes history before app)
  ...
  return (
    <div className="space-y-3">
      {/* Card — always inline, always live */}
      <div
        ref={cardRef}
